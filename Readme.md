...
- Cleaned size: 113,382 rows, 13 columns
...
| Spotify API year enrichment notebook | `spotify_trends_dashboard/spotify_api_year_enrichment.ipynb` |
...
4. Extracts or enriches release year through a staged fallback process:
   dataset year/release date, then the Spotify API year file generated by `spotify_api_year_enrichment.ipynb`.
...
10. Removes duplicate source records where identifiers are available and exports the final cleaned CSV.

## Spotify API Year Enrichment

The raw Kaggle file does not include a release year column. To avoid fake year values, use `spotify_trends_dashboard/spotify_api_year_enrichment.ipynb` to create:

```text
spotify_trends_dashboard/dataset_5a/spotify_api_track_years.csv
```

That notebook uses Spotify `track_id` values, requests real track metadata from Spotify, extracts `album.release_date`, and saves `track_id,Year`. The preprocessing notebook then uses this generated file as the source for `Year`.
...
```bash
export SPOTIFY_API_MAX_TRACKS=500
```
